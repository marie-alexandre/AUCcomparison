...
#' \code{Model_estimation} \tab either the vector of the marginal parameter estimates (at least for the groups \code{Group1} and \code{Group2}), or a list containing this vector labeled _'beta'_ among other arguments (see \link[AUCcomparison]{MEM_Polynomial_Group_structure} for details about the parameter order). \cr
...
#' Assuming a time interval \mjseqn{\[0,T_{g1}\]} for the group 1 and \mjseqn{\[0,T_{g2}\]} for the group 2, the difference of AUC is then calculated as
#' \mjsdeqn{\Delta AUC = \int_0^{T_{g_2}} E(\hat{Y_{g_2}})(t) dt - \int_0^{T_{g_1}} E(\hat{Y_{g_1}})(t) dt}
#' Similarly, the difference of the normalized AUC (\mjseqn{\Delta nAUC}) for these two same groups is then defined as 
#' \mjsdeqn{\Delta nAUC = \frac{1}{T_{g_2}}\int_0^{T_{g_2}} E(\hat{Y_{g_2}})(t) dt - \frac{1}{T_{g_1}}\int_0^{T_{g_1}} E(\hat{Y_{g_1}})(t) dt}
#' When we want to estimate the difference of AUC (or nAUC) on the same time interval for the two groups (\code{common.interval} = TRUE), we define \mjseqn{T = \min(T_{g_1},T_{g_2}))} and we calculate the difference of AUC as 
#' \mjsdeqn{\Delta AUC = \int_0^{T} E(\hat{Y_{g_2}})(t) dt - \int_0^{T} E(\hat{Y_{g_1}})(t) dt}
#' \mjsdeqn{\Delta nAUC = \frac{1}{T}\int_0^{T} E(\hat{Y_{g_2}})(t) dt - \frac{1}{T}\int_0^{T} E(\hat{Y_{g_1}})(t) dt}
#' 
