...
#' @description This function performs a one-way ANOVA to compare the area under the curves of multiple groups marginal dynamics, modeled by group-structured polynomials or B-spline curve in Mixed-Effects model.
...
#' @param Averaged a logical scalar. If TRUE, AUC are evaluated as normalized AUC (nAUC) where nAUC is computed as the AUC divided by the range of time of calculation. If FALSE (default), the classic AUC is calculated (see \link[AUCcomparison]{Group_specific_AUC_estimation} for more details).
...
#' @param bartlettTest a logial scalar indicating whether a bartettTest must be performed before the ANOVA. Default: FALSE
...
#'
#' @return A list of three elements elements:
#' \itemize{
